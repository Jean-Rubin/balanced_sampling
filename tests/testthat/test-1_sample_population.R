...
sample_test_size <- sum(population_test_1$pi_i)
...
sample_test_size <- sum(population_test_2$pi_i)
...
describe("jump_wr_ent()", {
  set.seed(1234)
  for (k in seq_len(3)) {
    jump_result_1 <- jump_wr_ent(X_1, population_test_1$pi_i, eps)
    ...
    jump_result_2 <- jump_wr_ent(X_2, population_test_2$pi_i, eps)
